 ...
 * Parses the ClaML DTD to an object model
 *
 */
public class ClaMLParser {
	...
	public ClaMLRoot parseClaMLFromXML(File input) {
		Element root ;
		
		try {
			root = ParserUtils.readXmlFromFile(input).getRootElement();
	...
	public ClaMLRoot parseClaMLFromXML(InputStream is) {
		Element root ;
		
		try {
			root = ParserUtils.readXmlFromFile(is).getRootElement();
	...
	private ClaMLRoot parseClaMLRoot(Element root){
		...
		for(Iterator<Element> i = root.elementIterator(); i.hasNext();){
			...
			if(e.getQualifiedName() == "Meta"){
				...
				claMLRoot.setTitle(new ClaMLTitle(e.attributeValue("date")
				...
				//if we can not find a place to put this child, add it at the root node
...
//			addChild(parent, child);
	...
	private boolean insertChild(ClaMLClass root, ClaMLClass child){
		...
		//if the parent has a child, that is a superclass of the child we want to insert --> add this child to the parents child.
...
//			parent.addChild(child);
		...
		//if we couldn't find parent on this layer, recursively search inner layers.
		else {
...
//				claMLClass.setParent(new ClaMLClass(e.attributeValue("code"), findSuper));
			}
	...
	private ClaMLModifiedBy parseModifiedBy(Element root) {
		ClaMLModifiedBy modifiedBy = 
				new ClaMLModifiedBy(claMLRoot.getModifiers().get(root.attributeValue("code")));
...
//				if this document is wellformed, we do not need to parse this here
			}
			else if(e.getQualifiedName() == "Rubric"){
